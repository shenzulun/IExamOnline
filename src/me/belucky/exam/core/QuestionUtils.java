...
import org.magen.tool.file.FileTools;
...
public class QuestionUtils {
	...
	public static void refreshCache(){
		...
		EasyUtil.putCache("test_questions_1", testQuestionList1);
		EasyUtil.putCache("test_questions_2", testQuestionList2);
		EasyUtil.putCache("test_questions_3", testQuestionList3);
		EasyUtil.putCache(TEST_QUESTIONS_MAP, cacheMap);
		log.info("试题库加载成功...");
		//判断是否写入数据库
...
//		result.add(new QuestionTypeDTO("5","历史"));
	...
	public static List<TestQuestionDTO> queryTestQuestionsBySeq(String userNo, String seqno, String questionType){
		Map<String,TestQuestionDTO> cacheMap = EasyUtil.getCache(TEST_QUESTIONS_MAP);
		...
		if("4".equals(questionType)){
			...
			list = EasyUtil.getCache("test_questions_" + questionType);
			if(seqno == null || "".equals(seqno)){
	...
	public static List<QuestionAnswerDTO> submitAnswers(String userNo, String checkAnswers){
		...
		for(String key : keys){
			...
			if(testDto.getType() == 1 || testDto.getType() == 2){
				...
				ansDto.setCorrect(CommonUtils.compareTwoString(correctAnswer, selectOption));
